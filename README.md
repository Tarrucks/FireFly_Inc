...
- **Drone verification state machine** (4 Hz) — IDLE → ENROUTE (snapshot coordinates) → ORBIT (200 m ring, 15 s) → RESOLVE (RED if confirmed hostile, GREEN if confirmed decoy, or inconclusive with the band unchanged when the contact has departed the snapshot area) → RTB. Confirm/deny is scripted from scenario ground truth, exactly as in the full stack's demo mode.
